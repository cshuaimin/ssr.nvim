...
-- List item per line
---@class Item
---@field fold_idx number which fold this line belongs to, 1-based
---@field match_idx number which match this line belongs to, 0-based, 0 for filename
...
---@params results { file: File, matches: Match[] }[]
function ResultList:set(results)
  self.folds = {}
  self.items = {}
  ...
  local lines = {}
  for fold_idx, result in ipairs(results) do
    local fold = Fold.new(fold_idx ~= 1, result.file, result.matches)
